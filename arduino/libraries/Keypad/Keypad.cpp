...
bool Keypad::updateList() {
	...
	// Delete any KP_IDLE keys
	for (byte i=0; i<LIST_MAX; i++) {
		if (key[i].kstate==KP_IDLE) {
			key[i].kchar = NO_KEY;
			key[i].kcode = -1;
			...
			if ((idx == -1) && button) {
				for (byte i=0; i<LIST_MAX; i++) {
					if (key[i].kchar==NO_KEY) {		// Find an empty slot or don't add key to list.
						key[i].kchar = keyChar;
						key[i].kcode = keyCode;
						key[i].kstate = KP_IDLE;		// Keys NOT on the list have an initial state of KP_IDLE.
...
/*
|| @changelog
|| | 3.1 2013-01-15 - Mark Stanley     : Fixed missing RELEASED & KP_IDLE status when using a single key.
